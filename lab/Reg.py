...
def localWeight(point,xmat,ymat): 
    wei = kernel(point,xmat)
    W = (X.T*(wei*X)).I*(X.T*(wei*ymat.T)) 
    return W

def localWeightRegression(xmat,ymat): 
    ypred = np.zeros(m) 
    for i in range(m):
...
k = 1.5         # increase k to get smooth curves
ypred = localWeightRegression(X,mtip)
print(ypred)
graphPlot(X,ypred)